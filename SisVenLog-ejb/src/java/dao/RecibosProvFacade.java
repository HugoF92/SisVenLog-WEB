...
public class RecibosProvFacade extends AbstractFacade<RecibosProv> {
    ...
    public List<RecibosProv> listadoRecibosProveedores() {
        String sql = "SELECT * FROM recibos_prov";
        Query q = em.createNativeQuery(sql, RecibosProv.class);
        List<RecibosProv> resultado = q.getResultList();
        return resultado;
    }
    ...
    public List<RecibosProv> obtenerPorCodEmprNroChequeCodBanco(short codEmpr, String nroCheque, short codBanco) {
        String sql = "SELECT rp FROM RecibosProv rp, "
                + "RecibosProvCheques rpc, "
                + "ChequesEmitidos ce "
                + "WHERE rp.recibosProvPK.nrecibo = rpc.recibosProvChequesPK.nrecibo "
                + "AND rp.recibosProvPK.codProveed = rpc.recibosProvChequesPK.codProveed "
                + "AND rp.recibosProvPK.codEmpr = rpc.recibosProvChequesPK.codEmpr "
                + "AND rpc.recibosProvChequesPK.nroCheque = ce.chequesEmitidosPK.nroCheque "
                + "AND rpc.recibosProvChequesPK.codBanco = ce.chequesEmitidosPK.codBanco "
                + "AND rpc.recibosProvChequesPK.codEmpr = ?1 "
                + "AND rpc.recibosProvChequesPK.nroCheque = ?2 "
                + "AND rpc.recibosProvChequesPK.codBanco = ?3";
        
        TypedQuery<RecibosProv> query = em.createQuery(sql, RecibosProv.class);

        query.setParameter(1, codEmpr);
        query.setParameter(2, nroCheque);
        query.setParameter(3, codBanco);
