...
@ManagedBean
@SessionScoped
public class LiHistoricoPrecios {
    ...
    public LiHistoricoPrecios() throws IOException {
    ...
    public void instanciar() {
        ...
        this.tiposVentas = new TiposVentas(new TiposVentasPK());
        this.listaTiposVentas = new ArrayList<TiposVentas>();
    ...
    public List<TiposVentas> listarTiposVentas() {
        this.listaTiposVentas = tiposVentasFacade.findAll();
        return this.listaTiposVentas;
    }
    ...
    public void ejecutar(String tipo) {
        ...
        if (this.tiposVentas != null) {
            tiposVenta = this.tiposVentas.getTiposVentasPK().getCtipoVta() + "";
            descTipoVta = tiposVentasFacade.find(this.tiposVentas.getTiposVentasPK()).getXdesc();
        }else{
            tiposVenta = "TODOS";
            descTipoVta = "TODOS";
        }
        ...
        if (tipo.equals("VIST")) {
            rep.reporteLiHistoricoPrecios(dateToString(desde), dateToString2(desde), tiposVenta, 
                    descTipoVta, "admin", tipo);
        } else {
            ...
            columnas[1] = "descripocion";
            columnas[2] = "relacion";
            columnas[3] = "gravado_exenta";
            columnas[4] = "estado";
            columnas[5] = "ctipo_vta";
            columnas[6] = "tipo_vta";
            columnas[7] = "iprecio_caja";
            columnas[8] = "iprecio_unidad";
            columnas[9] = "frige_desde";
            columnas[10] = "frige_hasta";

            String sql = "SELECT m.cod_merca, m.xdesc, m.nrelacion, m.mgrav_exe, m.mestado, \n"
                    + " p.ctipo_vta, tv.xdesc as xdesc_venta, p.iprecio_caja, p.iprecio_unidad \n"
                    + ", p.frige_desde, p.frige_hasta \n"
                    + "FROM precios p, mercaderias m,  tmp_mercaderias t, tipos_ventas tv\n"
                    + "WHERE frige_desde >= '"+fdesde+"'\n"
                    + "AND  p.cod_empr = 2\n"
                    + "AND p.cod_depo = 1\n"
                    + "AND p.ctipo_vta = tv.ctipo_vta\n"
                    + "AND p.cod_merca = m.cod_merca\n"
                    + "AND m.cod_merca = t.cod_merca\n"
                    + "AND (p.ctipo_vta = '"+tiposVenta+"' OR '"+tiposVenta+"'='TODOS')\n"
