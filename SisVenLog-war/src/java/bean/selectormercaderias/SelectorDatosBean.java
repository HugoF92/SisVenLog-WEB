...
public class SelectorDatosBean {
    ...
    @EJB
    private GenericFacadeSQL personalizedFacade;
    ...
    public void cargarLista() {
        ...
        personalizedFacade.executeQuery("truncate  table " + tabla_temporal);
        ...
        try {
            ...
            if (datos.getTarget().size() > 0) {
                ...
                for (int i = 0; i < listaDatosSeleccionadas.size(); i++) {
                    ...
                    personalizedFacade.executeQuery("INSERT INTO " + tabla_temporal + " (" + campos_tabla_temporal + " )\n"
                            + "	VALUES ('" + listaDatosSeleccionadas.get(i).getCodigo()
                            + "', '" + listaDatosSeleccionadas.get(i).getDescripcion() + "' )");
                }
            } else {
                personalizedFacade.executeQuery("insert into " + tabla_temporal + "\n"
                        + sql);
            }            
            this.listaDatosSeleccionadas = new ArrayList<TmpDatos>();
            datosFiltrados = true;
